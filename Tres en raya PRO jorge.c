...
int main(int argc, char** argv) {
    ...
    while(i==0){ j=0; ganar=0;
    do{printf("¿Quieres comenzar tú la partida?\n1.No\n0.Sí\n");
    scanf("%d", &turno);}while(turno!=0 && turno!=1);
    imprimir(tablero);
    ...
    for(j=0, i=0; !(j==3 && i==0);i=(i+1)%3){
        tablero[j][i]=' ';
        if(i==2)
            j++;} //Vaciamos el tablero
    do{printf("¿Volver a jugar?\n0.Sí\n1.No\n");
    scanf("%d", &i);}while(i!=0 && i!=1);}
    return (EXIT_SUCCESS);
}
